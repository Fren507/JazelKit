...
- [ ] **`JazelKitConfig.ts`**: Standardwerte für alle Konfigurationsoptionen dokumentieren.
- [ ] **`build.ts`**: Fehlerbehandlung für fehlende Dateien verbessern.
- [ ] **`helpers.ts`**: Zusätzliche Hilfsfunktionen für häufige Aufgaben implementieren.
- [ ] **`layout.ts`**: Unterstützung für verschachtelte Layouts erweitern.
